...
/* MATCHED (W-convergence). Forever BoxBarSoundOn-when-not-in-range loop.
 * Two levers cracked the ex-rc8 cmov-vs-branch residual:
 *  1. goto-to-END for the `flag=1` arm (label `s:` placed after `continue`)
 *     keeps the inner test a real conditional BRANCH: jump.c's cond-move
 *     if-conversion (HAVE_conditional_move block) only fires when the store is
 *     the lone insn immediately before the join label, so exiling it defeats
 *     the `movn a2,zero,v1` fold. reorg then annuls it into the `beql v1,zero;
 *     addiu a2,1` branch-likely (a2=1 in the taken delay).
 *  2. `int flag` + explicit `flag & 0xff` reproduces ROM's `andi a2,0xff`: a
 *     `char flag` gets PROMOTE_MODE'd to SImode and gcc proves it in {0,1},
 *     dropping the zero-extend; the int+mask keeps the byte test. */
extern unsigned int _ACTWait(int a0);

void func_00172FC8(void *volatile a0) {
    int flag;
    for (;;) {
    ...
    g:
        if (!(flag & 0xff)) BoxBarSoundOn(a0, 0x160);
        _ACTWait(1);
        continue;
    s:
        flag = 1;
...
void func_001731A0(volatile int a0) {
    ...
    debug_StdPrintfDummy(D_00553D90);
    *(int *)(gobj + 0x30) = 1;
    _ACTWait(0);
}
...
void func_00173370(volatile unsigned int a0)
{
    ...
    int *s0;
    s0 = *((int **) (a0 + 0x164));
    debug_StdPrintfDummy((char *)D_00553DF0);
    s0[0x30 / 4] = 0xF;
    _ACTWait(0);
}
