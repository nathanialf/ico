...
order: `.text`, `.vutext`, `.data`, `.vudata`, `.rodata`, `.lit4`, `.sdata`,
`.sbss`, `.bss`. The last two are NOBITS — they hold no ROM bytes — so their
figure is **ownership**: how much of the section a compiled TU defines and the
link seats at the ROM's addresses, not bytes reproduced. `.vudata` is empty in
this ELF and badges as `-`.
