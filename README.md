...
<!-- progress:begin -->
![.text progress](https://img.shields.io/badge/text-9.36%20%25-orange.svg)
...
`config/ico.<slug>.yaml` (`tools/ico_version.py`). Matched functions flow
between targets via `tools/port_from_aug6.py` (see
[`decomp/PORTING.md`](decomp/PORTING.md)).
...
`da3644c54c26fe760f3b6a591a5fc2eab396ed2b`. The PAL target is being re-seeded
from the USA retail work, so the round-trip is not byte-identical yet.
...
1. All 216 game TUs are already carved (`config/ico.pal.yaml`); every
