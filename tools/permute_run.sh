...
# No USE_MODERN_AS: the modern-gas path was retired 2026-08-05 (see
# compile_c.sh). A permuter candidate that only assembles/matches under modern
# gas is a false positive — it would be matching the assembler's delay-slot
# filling, not a source shape.
...
# FILE_OFF=${FILE_OFF_HEX}  postprocesses=${PP_FLAGS:-(none)}  assembler=period-only
set -e
INPUT="\$1"
shift || true
OUTPUT=""
