...
EEGCC="${EEGCC:-${PROJECT_ROOT}/tools/cc/ee-gcc2.96/bin/gcc}"
EEGCC_LIB="${EEGCC_LIB:-${PROJECT_ROOT}/tools/cc/ee-gcc2.96/gcc-lib/ee/2.96-ee-001003-1/}"
AS_FOR_QD="${AS_FOR_QD:-mips-linux-gnu-as}"

if [ ! -x "${EEGCC}" ]; then
    echo "ERROR: ee-gcc 2.96 not found at ${EEGCC}." >&2
...
# --- compile.sh — ee-gcc 2.96 → mips-linux-gnu-as → .o ----------------------
