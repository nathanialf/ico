...
#   ee-gcc 2.9-991111
#   → always-on ROM-parity awk rules (fcc nop, jr/j-$31 FP-delay nop)
#   → $sp/$ra/$zero/etc → $29/$31/$0/etc sed translation
#   → the period assembler ee-as 2.9-991111 ONLY (no modern-gas
#     fallback, retired 2026-08-05) + objcopy section alignment
...
# lookup (include_ito.txt) matches the real build. Place it in
...
# No per-func/per-file postprocess allowlists and no USE_MODERN_AS. The
# postprocess machinery (config/{no_trailing_nop,shared_sp_restore,
# shared_jr_restore,early_body_swap,swap_addu_operands,coalesce_v1_v0}.txt plus
# their postprocess_*.py) is a RETIRED, banned matching lever — a regalloc /
# scheduling diff is a source-shape mismatch (CLAUDE.md "Crutches are BANNED").
# The gates and their scripts were deleted 2026-09-04. Modern gas went
# 2026-08-05: a candidate that only matches under it is matching the
# assembler's delay-slot filling, not a source shape.
#
# What survives below is ALWAYS-ON ROM-encoding parity only: the fcc-hazard nop,
# the jr/j-$31 FP-delay nop, and the reg-name -> numbered translation.
...
# FILE_OFF=${FILE_OFF_HEX}  assembler=period-only, no postprocess allowlists
set -e
INPUT="\$1"
shift || true
OUTPUT=""
...
# 2. Reg-name → numbered translation (always applied; the period ee-as needs it).
cat <<'EOF'
...
# 4. Assemble with the period assembler ONLY — no modern-gas fallback (retired
#    2026-08-05, see compile_c.sh). A candidate that assembles only under modern
#    gas would score as a match on the assembler's delay-slot filling rather than
#    on source shape, which is exactly the false positive this permuter is meant
#    to avoid producing. Errors surface instead of being swallowed by 2>/dev/null.
cat <<EOF
if ! "${EE_AS_BIN}" ${EE_ASFLAGS} -o "\${OUTPUT}" "\${TMP_S}"; then
    echo "permuter: period assembler REJECTED \${TMP_S} (no modern-gas fallback)" >&2
    exit 1
fi
