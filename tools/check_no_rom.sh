...
for f in "${files[@]}"; do
    ...
    # docs/progress.json is the GitHub Pages dashboard's data, generated by
    # tools/progress_tree.py from the exempt symbol_addrs file + INCLUDE_ASM
    # directives. It holds only address-derived function names, integer byte
    # sizes, and matched/unmatched booleans — no ROM bytes. Large (>256 KiB)
    # only because the aug6 build is fully named (5k+ functions). Content is
    # still subject to the byte-array rules below; exempt the size cap.
    case "$f" in docs/progress.json) continue ;; esac
    size=$(wc -c < "$f")
