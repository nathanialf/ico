#!/usr/bin/env bash
# auto_permute_parked.sh — parallel permuter pool over tools/parked.txt.
#
# Iterates each function listed in tools/parked.txt and runs
# tools/permute_run.sh against it. Up to PARALLEL functions are permuted
# concurrently (default 4), each pinned to `-j 1` so the total concurrent
# thread count equals PARALLEL — i.e. each core handles its own function.
#
...
#
# Usage:
#   tools/auto_permute_parked.sh             # parallel pool over parked.txt
#   tools/auto_permute_parked.sh func_X      # only this one (single, not parallel)
#
# Env:
...
#   PARKED_FILE=path  override path to parked.txt (default:
#                      tools/parked.txt).
#
# Output:
...
#   lib/decomp-permuter/auto_parked.log

set -uo pipefail

PROJECT_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PARKED_FILE="${PARKED_FILE:-${PROJECT_ROOT}/tools/parked.txt}"
...
ACTIVITY_LOG="${PERMUTER_DIR}/auto_parked.log"
...
if [ ! -f "${PARKED_FILE}" ]; then
    echo "ERROR: parked.txt not found at ${PARKED_FILE}" >&2
    exit 1
fi
...
# --- parked.txt parsing -----------------------------------------------------
# Format: one func name per line; '#' starts a comment; blank lines OK.
read_parked() {
    awk '
        { sub(/#.*/, ""); gsub(/^[ \t]+|[ \t]+$/, "") }
        $0 != "" { print }
...
find_seed() {
    ...
    local candidates=(
        "${PROJECT_ROOT}/tough_nuts/${name}/${name}.c"
        "${PROJECT_ROOT}/tough_nuts/${name}/${file_off_hex}.c"
        "${PROJECT_ROOT}/tough_nuts/${name}/${file_off_lower}.c"
    ...
    if [ -d "${PROJECT_ROOT}/tough_nuts/${name}" ]; then
        local first
        first="$(find "${PROJECT_ROOT}/tough_nuts/${name}" -maxdepth 1 -name '*.c' | head -1)"
        if [ -n "${first}" ]; then
            echo "${first}"
            return 0
        fi
...
# --- collect candidates from parked.txt -------------------------------------
collect_candidates() {
    local arg="${1:-}"
    if [ -n "${arg}" ]; then
        echo "${arg}"
...
while :; do
    ...
    if [ "${#CANDIDATES[@]}" -eq 0 ]; then
        ...
        log "no remaining candidates in ${PARKED_FILE} — exiting."
        break
    fi
