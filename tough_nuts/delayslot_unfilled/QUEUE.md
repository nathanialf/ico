...
|---|----|----------|----------|-------|------------------|--------------------------|--------|
| 1 | src/cod/vendor_2418A0.c | sceVu0CopyVector | 0x00243B60 | 4 | `jr ra` | `sq a2,0(a0)` | MATCHED (hand asm, jr-slot sq) |
| 2 | src/cod/vendor_2418A0.c | sceVu0CopyMatrix | 0x00243B70 | 10 | `jr ra` | `sq t1,48(a0)` | MATCHED (hand asm, jr-slot sq) |
| 3 | src/cod/vendor_2418A0.c | sceDmaPutStallAddr | 0x00244958 | 10 | `beq a0,v1,<+0x24>` | `lw v0,0(v0)` (also: period pads a 2nd trailing nop) | OPEN — parked C is CORRECT but needs a filled slot; source shape still unknown |
...
| 5 | src/Packet.c | pac_makeMaterialTable | 0x0011A2A8 | 18 | `j debug_StdPrintfDummy` (tail call) | `lw a2,4(v0)` | MATCHED (plain C, EABI 5th arg in `$8`) — see HANDOFF_Packet.md |
| 6 | src/Packet.c | pac_makeMaterialTableLine | 0x0011A2F0 | 18 | `j debug_StdPrintfDummy` (tail call) | `sw a1,44(v1)` | MATCHED (plain C, `int`-typed cursor store defeats DSE) — see HANDOFF_Packet.md |
| 7 | src/cod/vendor_2418A0.c | sceVu0TransposeMatrix | 0x002439B0 | 18 | `jr ra` | `sq t3,48(a0)` | MATCHED (hand asm, MMI transpose) |
| 8 | src/cod/vendor_2418A0.c | sceVpu0Reset | 0x00244598 | 24 | `jr ra` | `sq v1,0(a1)` | MATCHED (real C, volatile-first/plain-second store pair) |
...
- #3 (`sceDmaPutStallAddr`): read-modify-write of 0x1000E060; ROM hoists the
...
- #5/#6 CONFIRMED that reading (2026-08-04, HANDOFF_Packet.md): with the
  parked C's `volatile` dropped, gcc filled BOTH `j debug_StdPrintfDummy`
...
- vendor_2418A0: `sceVu0TransposeMatrix` / `sceVu0CopyVector` keep C prototypes in the
...
`sceDmaPutStallAddr` and `_sysbitNext` were briefly landed by assembling their TUs
...
are exactly that). For `_sysbitNext` the known obstruction is that a DI->SI
...
**Worth revisiting under 2.96:** rows #1/#2/#7 (`sceVu0CopyVector`,
`sceVu0CopyMatrix`, `sceVu0TransposeMatrix`) were landed as whole-function HAND ASM on the
...
## 2026-08-05 — CORRECTION to row #8's recorded mechanism (sceVpu0Reset)

Commit `4b302110`'s message explains `sceVpu0Reset`'s delay-slot `sq` as a
...
- The TImode exemption is mode-specific, so it does NOT help byte/half/word stores
  (`kputchar`'s store is QImode, `sceDmaPutStallAddr`'s load is SImode).
...
template:
- `0x254d1c` (`_ipuSetMPEG1`): a genuine two-use pointer (read-modify-write).
- `0x2445f0` (`sceVpu0Reset`): the TImode address rule above.
- `0x241cac` (`sceGsResetPath`): the function is ONE basic block, so gcse returns early
...
## 2026-08-05 — CORRECTION to kputchar's recorded mechanism (commit 56df1a4e)

`56df1a4e` landed kputchar and explained it as: cse rewrites the `base | offset`
...
This is the same KIND of mechanism as sceVpu0Reset's: an address form the target
...
it knows the pseudo's value in the same extended basic block. In kputchar the
...
### It does NOT transfer to sceDmaPutStallAddr, and here is the specific reason

ROM pins sceDmaPutStallAddr's non-volatile register-form `lw` into the ENTRY block, where
