...
- #4-#6 are plain-C void tail calls (`j <extern>`); gcc 2.9 never emitted a
  bare-slot `j` for the ROM here — the real source shape presumably made gcc
  fill the slot itself (e.g. different statement order producing the
  store/load last).
