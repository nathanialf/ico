# scePadInfoPressMode (common/src/PObj) — parked rc7 after 30-stall
...
ROM: daddu a2,a0 (SAVE orig a0->a2); addiu v1,0x70; addiu a0,0x1C (REUSE a0 reg as the 0x1C const);
     ...
     ... scePadGetButtonMask(a2 = orig a0).
Built: keeps a0 (orig), uses a1 as the a1*0x1C accumulator and v0 as the 0x1C const; index in a1.

## Root: scePadGetButtonMask(a0) keeps a0 live across the 2D index. ROM sacrifices a0 (saves to a2) and
...
2D-array D[a0][a1].f10 (rc7=best, dev idiom, matches sibling scePadGetDmaStr style); explicit
...
in scePadInfoPressMode.c now) — it already has mult1+save; only the a1<->a2 rename remains, so a
