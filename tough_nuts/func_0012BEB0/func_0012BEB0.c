...
void func_0012BEB0(short *_a0, short *_a1) {
    register short *a0 __asm__("$10") = _a0;
    register short *a1 __asm__("$11") = _a1;
    register int blk_sz __asm__("$7") = 0xC;
    register unsigned int *fmt_tab __asm__("$6");
    int fmt_idx;
    unsigned int *entry;
    fmt_tab = D_00276268;
    KEEP_LIVE(a0);
    ...
    KEEP_LIVE(blk_sz);
    KEEP_LIVE(fmt_tab);
    fmt_idx = *((unsigned char *)a0 + 0x12) & 0x3F;
    entry = &fmt_tab[fmt_idx * 3];
    if (entry[1] == 0) __asm__("break 0,7");

    *(short *)((char *)a1 + 0xEA) = 8;

    {
        int idx2 = *((unsigned char *)a0 + 0x12) & 0x3F;
        unsigned int *entry2 = &fmt_tab[idx2 * 3];
        unsigned int fmt0 = entry2[0];
        unsigned short width = *(unsigned short *)((char *)a0 + 0x14);
        int blk;
        if (fmt0 >= 0x3B) {
            blk = 0;
        } else {
    ...
    {
        unsigned short n_levels = *(unsigned short *)((char *)a1 + 0xE0);
        if (n_levels != 0) {
            int level = 0;
            unsigned char lbu13 = *((unsigned char *)a0 + 0x13);
            do {
                int s_off = level * 0x24;
                int blk_size = lbu13 * 0xC;
                unsigned short tw = *(unsigned short *)((char *)a0 + 0x14);
                unsigned short th = *(unsigned short *)((char *)a0 + 0x16);
                unsigned int *p = (unsigned int *)((char *)D_00276268 + blk_size);
                char *out_base = (char *)a1 + s_off;
                int w_shr = (int)tw >> level;
                int h_shr = (int)th >> level;
                int wh = w_shr * h_shr;
                if (p[1] == 0) __asm__("break 0,7");
                {
                    int q = wh / (int)p[1];
                ...
                {
                    unsigned char lbu13_in = *((unsigned char *)a0 + 0x13);
                    unsigned short tw_in = *(unsigned short *)((char *)a0 + 0x14);
                    unsigned int *p2 = (unsigned int *)((char *)D_00276268 + lbu13_in * 0xC);
