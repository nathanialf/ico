...
**Reason parked:** iter 4: 170 byte diffs out of 104. Complex 59-entry GS pixel format jtbl with multi-scope local vars (a0/a1 saved to t2/t3 via 'daddu $10/$11, ...'), 12-byte D_00276268 entries, double-jtbl in loop, assertion via break 0,7. C draft has structural shape but lots of reg/scheduling diff. Tried REG pins on a0,a1,blk_sz=12,fmt_tab + KEEP_LIVE; gcc still picks different reg/order. Needs deeper structural rewrite — possibly the original used specific local var patterns / loop body restructuring.
