...
**Reason parked:** rc7 near-miss: GObjCreate. Structure nailed (28->7). Remaining: s0=v0 slot-copy placement (expected between _164/_170 stores) + return-value v0=s0 setup placement (expected between _4/_8 stores). Recipe: i=$4 pin + scoped ANCHOR(i)+i++ (block 1-fold), nl=$5 n-copy for loop bound, pointer-walk p++ (not base+i*stride), prod=$2 + compound prod+=base (addu dest). E9E0 is the twin (calls func_0013E0C0, omits final _C=0 store).
