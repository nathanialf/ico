...
extern float D_00629024;
extern float D_0062C504_v[] __asm__("D_0062C504");
extern float D_0027E930[4][4];
extern float D_0027E970[4][4];
extern float D_0027E9B0[4];

int enemy_dodge(float *out, void *p1, void *p2) {
...
La68:
    ret = HandyCamera_TargetMoveType(p1, p2);
    if (!(ret < D_00629024)) goto Laa8;
La84:
    out[0] = D_0027E9B0[0];
    out[1] = D_0027E9B0[1];
    out[2] = D_0027E9B0[2];
...
Laac:
    ...
    {
        int i, best;
        best_val = D_0062C504_v[0];
        best = -1;
        for (i = 0; i < 4; i++) {
            float d = HandyCamera_TargetMoveType(p1, D_0027E930[i]);
...
Lb40:
    {
        ...
        best = -1;
        for (i = 0; i < 4; i++) {
            float d = HandyCamera_TargetMoveType(p2, D_0027E930[i]);
            if (best_val < d) { best_val = d; best = i; }
        }
        if (best == -1) goto Lend;
        ret = pac_DispQW();
        rv = 1;
