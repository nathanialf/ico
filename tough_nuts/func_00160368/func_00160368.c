...
void func_00160368(void *self) {
    ...
    if (de4 == 0) return;
    r0 = ContinueCorrectPosition(de4);
    ...
    if (val < 0x17 && val >= 0x14) {
        cond = 1;
    } else if (*(int *)(*(int *)(*(int *)((char *)de4 + 0x164) + 0x678) + 0x368) > 0) {
        cond = 1;
    } else {
        cond = 0;
    }
    if ((cond & 0xFF) != 0) return;

    base = *(int **)((char *)self + 0x164);
    g = *(int **)((char *)base + 0x670);
    if (*(int *)((char *)g + 0x1E0) != 0 &&
        ...
        *(long long *)((char *)base + 0x20) |= 1;
    } else {
        BoxBarSoundOn(self, 0x100);
    }
}
