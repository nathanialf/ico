...
/* m2c scaffold from asm/aug6/nonmatchings/omori/src/camera-editor/brainAddLevel.s (target mipsel-gcc-c, context-free).
 * NOT a match — reshape into a goto-CFG-mirror + recover intent (see decomp-match skill). */
void brainAddLevel(void *a0, float arg0) {
    float *p = (float *)a0;
    float prod = p[4] * arg0;
    float v = p[1] + prod;
    p[1] = v;
...
INCLUDE_ASM("asm/aug6/nonmatchings/omori/src/camera-editor", brainSetLevel);
