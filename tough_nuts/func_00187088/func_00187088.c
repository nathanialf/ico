...
void func_00187088(void *a0, float *a1, int a2) {
    ...
    if (FSqrt(*(float *)((char *)sub + 0x120) * *(float *)((char *)sub + 0x120) + *(float *)((char *)sub + 0x128) * *(float *)((char *)sub + 0x128)) > 3.0f) {
        sceVu0SubVector(dst, src, D_00287140);
        if (FSqrt(dst[0] * dst[0] + dst[1] * dst[1] + dst[2] * dst[2]) < 1.5f) {
            D_00287140[0] = src[0];
            D_00287140[1] = src[1];
            D_00287140[2] = src[2];
        } else {
            sceVu0Normalize(dst, dst);
            func_00240038_v(dst, dst, 1.5f);
            sceVu0AddVector(D_00287140, D_00287140, dst);
        }
    }
    a1[0] = D_00287140[0];
    a1[1] = D_00287140[1];
