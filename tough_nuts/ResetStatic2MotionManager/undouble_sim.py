...
# apply undoubling to specified regs
undouble=[int(x) for x in sys.argv[1:]] if len(sys.argv)>1 else [986,2618,2619,1009]
for r in undouble:
    if r in reg: reg[r][1]=max(1,reg[r][1]//2)
def prio(a):
    r,l=reg.get(a,(0,1)); return (math.floor(math.log2(r)) if r>0 else 0)*r*10000//max(l,1)
# resort order by priority desc, tie by allocno
