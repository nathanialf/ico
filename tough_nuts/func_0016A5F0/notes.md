...
**Reason parked:** best real_count=3 (was 11). Structure+regs byte-correct; residual is ONE first-scheduler load-hoist in the tail-call epilogue: chain-int (a1->0x15C) hoisted ~3 slots before the a0-restore (barrier form) OR a0/a2 arg-setup floats 1 slot before the last zero-store (no-barrier form). Exhausted ~26 hand levers + lever.py pin sweep ($2 uniquely best) + reorder sweep (orig best) + -fno-schedule-insns (breaks siblings) + -fno-schedule-insns2 (worse). Seed = barrier + base REG($2). Pure scheduling -> permuter.
