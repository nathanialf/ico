...
extern float InitHandCameraCorrect(int a0, int a1);
extern int func_00260340(float f);
extern int func_0025EF78(int a, long long b);
extern float func_0025F748(int a);
extern int func_00191D90(void *a0, void *a1);
extern long long D_00553338, D_00553340, D_00553348;
extern float D_00629014, D_00629018, D_0062901C, D_00629020;

int subEnemyBrainMain(int self, int a1, int a2) {
    float idle[4];
    float mtx[4];
    float f22, f20, f21;
    float t1, t2, t3, t4;
    int *g;
    long long flag;
    int state, r, c5, r0, r1, deadbit;
    float f1v, f0v;
    int rv = 0;

    f22 = InitHandCameraCorrect(a2, ContinueCorrectPosition(a1));
    if (((float *)a2)[1] - ((float *)ContinueCorrectPosition(a1))[1] < 0.0f)
        f20 = -(((float *)a2)[1] - ((float *)ContinueCorrectPosition(a1))[1]);
    else
        f20 = ((float *)a2)[1] - ((float *)ContinueCorrectPosition(a1))[1];

    f21 = 0.0f;

    /* factor1: 270.0, D_00629014, D_00553338, with funcs */
    g = *(int **)((char *)(*(int **)(self + 0x164)) + 0x670);
    t1 = 270.0f;
    state = g[0x1E0 / 4];
    if (state < 0) goto after1;
    if (state < 2) goto a1;
    if (state == 2) goto b1;
    goto after1;
a1:
    flag = *(long long *)((char *)g + 0x208);
    t1 = *(float *)((char *)g + 0x1D8) * 270.0f;
    if ((int)((flag << 31) >> 32) & 1) {
        r = func_0025EF78(func_00260340(t1), D_00553338);
        t1 = func_0025F748(r);
    }
    goto after1;
b1:
    t1 = (*(float *)((char *)g + 0x1D8) * D_00629014) * 270.0f;
after1:;

    if (t1 < f22) {
        f21 = 1.0f;
    } else {
        /* factor2: 200.0, D_00629018, D_00553340, with funcs */
        g = *(int **)((char *)(*(int **)(self + 0x164)) + 0x670);
        t2 = 200.0f;
        state = g[0x1E0 / 4];
        if (state < 0) goto after2;
        if (state < 2) goto a2;
        if (state == 2) goto b2;
        goto after2;
    a2:
        flag = *(long long *)((char *)g + 0x208);
        t2 = *(float *)((char *)g + 0x1D8) * 200.0f;
        if ((int)((flag << 31) >> 32) & 1) {
            r = func_0025EF78(func_00260340(t2), D_00553340);
            t2 = func_0025F748(r);
        }
        goto after2;
    b2:
        t2 = (*(float *)((char *)g + 0x1D8) * D_00629018) * 200.0f;
    after2:;
        if (t2 < f20) f21 = 1.0f;
    }

    /* factor3: 120.0, D_0062901C, no funcs */
    g = *(int **)((char *)(*(int **)(self + 0x164)) + 0x670);
    t3 = 120.0f;
    state = g[0x1E0 / 4];
    if (state < 0) goto after3;
    if (state < 2) goto a3;
    if (state == 2) goto b3;
    goto after3;
a3:
    flag = *(long long *)((char *)g + 0x208);
    t3 = *(float *)((char *)g + 0x1D8) * 120.0f;
    deadbit = (int)((flag << 31) >> 32) & 1;
    goto after3;
b3:
    t3 = (*(float *)((char *)g + 0x1D8) * D_0062901C) * 120.0f;
after3:;

    if (f22 < t3) {
        /* factor4: 100.0, D_00629020, D_00553348, with funcs */
        g = *(int **)((char *)(*(int **)(self + 0x164)) + 0x670);
        t4 = 100.0f;
        state = g[0x1E0 / 4];
        if (state < 0) goto after4;
        if (state < 2) goto a4;
        if (state == 2) goto b4;
        goto after4;
    a4:
        flag = *(long long *)((char *)g + 0x208);
        t4 = *(float *)((char *)g + 0x1D8) * 100.0f;
        if ((int)((flag << 31) >> 32) & 1) {
            r = func_0025EF78(func_00260340(t4), D_00553348);
            t4 = func_0025F748(r);
        }
        goto after4;
    b4:
        t4 = (*(float *)((char *)g + 0x1D8) * D_00629020) * 100.0f;
    after4:;
        if (f20 < t4) f21 = -1.0f;
    }

    idle[0] = subCommonIdle((void *)a1)[0];
    idle[1] = subCommonIdle((void *)a1)[1];
    idle[2] = subCommonIdle((void *)a1)[2];
    r0 = ContinueCorrectPosition(self);
    r1 = ContinueCorrectPosition(a1);
    func_00191FD0(mtx, r0, r1);
    r = func_00191D90(idle, mtx);

    f1v = (r < 0x4B) ? 1.0f : 0.0f;
    c5 = (r < 0x65) ? 0 : 1;
    f0v = 0.0f;
    if (f1v != 0.0f) {
        if (*(int *)(*(int *)(a1 + 0x164) + 0x30) == 0xF) f0v = 1.0f;
    }
    if (f21 < 0.0f) rv = 1;
    if (0.0f < f21) rv = 2;
    if (f0v != 0.0f) rv = 3;
