...
**Reason parked:** rc12 2-call FP wrapper near-miss. void f(a0,a1){float buf[4]; memset(buf,0,0x10); buf[2]=1.0f; p=*(int**)(a1+0x15C); sceVu0ApplyMatrix(a0,p[3],buf);}. Structure correct; the cascading root is an s0/s1 swap: ROM maps a0->s0,a1->s1 (natural), gcc maps a0->s1,a1->s0. Cause: both params are saved to callee-saved across memset; gcc gives s0 to the param with the EARLIER last-use. ROM uses a0 first (sceVu0ApplyMatrix arg0) then a1 (p=a1->0x15C), but gcc evaluates the p/a1 arg before a0 (arg-eval order), making a1's last-use earlier -> a1->s0. 3 source orderings (inline/explicit-p/p-early) don't flip it. Sibling GetGirlPositionAtThisStage is single-param (no s0/s1). Needs a regalloc lever/permuter. NOT a floor, stall ~3.

**TU:** `src/act-parallel-control.c`

**Seed:** `tough_nuts/_GetRootObjectOrient/func_0014B708.c`

Disassembly:

```
.align 3
nonmatching _GetRootObjectOrient, 0x5C

glabel _GetRootObjectOrient
    ...
    /* 4B728 0014B728 7690090C */  jal        memset
    ...
    /* 4B748 0014B748 2E0E090C */  jal        sceVu0ApplyMatrix
