...
{
    int *sub = (int *)self[0x164 / 4];
    int *q = (int *)sub[0x670 / 4];
    unsigned int *p;
    unsigned int field, v0;
    if (q[0x1DC / 4] != 0) {
        int idx = self[2];
        p = (unsigned int *)(D_002A4C48 + idx * 0x4C);
        field = p[0x48 / 4];
        v0 = (field >> 18) & 1;
        if (v0 != 0) goto zero;
        v0 = (field >> 21) & 1;
...
} S_bl_00558E40;  /* stride 0x4, 1 accesses */

typedef struct {
    unsigned int     f_0;  /* 0x0  x1 */
} S_bl_00559000;  /* stride 0x4, 1 accesses */

typedef struct {
    unsigned int     f_0;  /* 0x0  x1 */
} S_bl_00559130;  /* stride 0x4, 1 accesses */

typedef struct {
    unsigned int     f_0;  /* 0x0  x1 */
} S_bl_00559150;  /* stride 0x4, 1 accesses */
