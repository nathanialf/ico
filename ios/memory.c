...
void iosMallocInitPartition(int a0, int a1) {
    ...
    debug_assertMessage(D_00557458, a0);
    if (node == 0) {
        return;
    }
    do {
            ...
            if (r == 0) {
                debug_assertMessage(D_00557470, node - a1, D_006A6970);
                r = 0xB;
                goto delay;
            }
            r = func_00265024((int *)node, D_00556F80);
            if (r == 0) {
                debug_assertMessage(D_00557480, node - a1);
                r = 0xB;
                goto delay;
            }
            r = func_00265024((int *)node, D_00557060);
            if (r != 0) {
                debug_assertMessage(D_005574A8, node - a1, node);
                return;
            }
            debug_assertMessage(D_00557490);
            r = 0xB;
        delay:
            do {
                r--;
