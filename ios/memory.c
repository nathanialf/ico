...
int iosMallocDebug(int *a0, int a1)
{
    if (a0 == 0)
    {
        debug_StdPrintfDummy(D_00556FB8);
        return 0;
    }
    if (strcmp(a0, D_00556F70) != 0)
    {
        debug_StdPrintfDummy(D_00556FD8);
        return 0;
    }
...
void _iosFreeWithFill(int *a0, int a1, int a2)
{
    ...
    debug_StdPrintfDummy(D_005572B8, a1, a2, a0, end);
    {
...
void *iosMallocCheckLeak2(void *ptr)
{
    ...
    debug_StdPrintfDummy(D_005572D8);
    if (ptr == 0) {
        debug_StdPrintfDummy(D_005572E8);
        __asm__ __volatile__("break");
        func_001AD748(D_005570E0, 0x334, D_00557300);
        __assert(D_005570E0, 0x334, D_006320B8);
        return 0;
    }
    prev = (IosMemNode *)((char *)ptr - 0x10);
    next = ptr;
    if (strncmp(prev, D_006320C8, 5) == 0) {
        n = atoi((char *)ptr - 0xB);
        *((char *)ptr - 0x10) = 0;
        next = (IosMemNode *)((char *)prev - (n - 0x10));
    }
    node = (IosMemNode *)((char *)next - 0x40);
    if (strcmp((int *)node, D_00557220) != 0) {
        sprintf(buf, D_00557320, node->prev, node, node->next);
        func_001AD748(D_005570E0, 0x344, buf);
        __assert(D_005570E0, 0x344, D_006320B8);
        return 0;
    }
    next = node->next;
    prev = node->prev;
    if (prev != 0) {
        if (strcmp((int *)prev, D_00556F80) == 0) {
            if (next != 0) {
                if (strcmp((int *)next, D_00556F80) == 0) {
                ...
                } else if (strcmp((int *)next, D_00557220) == 0) {
                    prev->next = next;
                    next->prev = prev;
                } else {
                    func_001AD748(D_005570E0, 0x389, D_00557370);
                    __assert(D_005570E0, 0x389, D_006320B8);
                    return 0;
                }
            } else {
                prev->next = 0;
        ...
        if (strcmp((int *)prev, D_00557220) != 0) {
            goto err_3bf;
        }
    }
    if (next == 0) {
    ...
    if (strcmp((int *)next, D_00557220) == 0) {
        node->free_prev = 0;
...
tail_node:
    ...
    __assert(D_005570E0, 0x3C2, D_006320B8);
    goto tag_free;
err_3b5:
    func_001AD748(D_005570E0, 0x3C5, D_00557370);
    __assert(D_005570E0, 0x3C5, D_006320B8);
    return 0;
tag_free:
...
err_3bf:
    ...
    __assert(D_005570E0, 0x3CF, D_006320B8);
ret_ptr:
    return ptr;
}
...
void iosMallocInitPartition(int a0, int a1) {
    ...
    debug_StdPrintfDummy(D_00557458, a0);
    if (node == 0) {
        return;
    }
    do {
            node += a1;
            strncpy(D_006A6970, node + 0x10, 0xF);
            D_006A6970[0xF] = 0;
            r = strcmp((int *)node, D_00557220);
            if (r == 0) {
                debug_StdPrintfDummy(D_00557470, node - a1, D_006A6970);
                r = 0xB;
                goto delay;
            }
            r = strcmp((int *)node, D_00556F80);
            if (r == 0) {
                debug_StdPrintfDummy(D_00557480, node - a1);
                r = 0xB;
                goto delay;
            }
            r = strcmp((int *)node, D_00557060);
            if (r != 0) {
                debug_StdPrintfDummy(D_005574A8, node - a1, node);
                return;
            }
            debug_StdPrintfDummy(D_00557490);
            r = 0xB;
        delay:
            do {
                r--;
