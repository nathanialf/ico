...
2. **The only sections gcc auto-emits into a TU's `.rodata` are switch jump
...
| `src/attackhit` | `[0x454B10]` | 0x554B10..0x554C00 | `jtbl_00554B10` (AttackCheckHit, 60) |
...
| `src/gv` | `.lit4` | `[0x5391EC]` | 0x6391EC..0x6391F0 | 4 | 1 (GetMatrixDirectionToZ's pi; the TU's other four pi words stay blob-owned extern reads until the pool re-carve) | interim |
...
### 2026-09-14 mig-lit4 pass 1: quaternion and tableSin pools
- `[0x538BB8, .lit4, ico2/sugipon/src/quaternion]`: D_00638BB8, one word 0.05f, GetSlerpQuaternionNoRegularize's sine floor, now the literal in the body, VMA 0x638BB8..0x638BBC, 4 B. The TU's whole pool: pool.o's carve ends at 0x638BB8 and tableSin's words follow.
- `[0x538BBC, .lit4, ico2/sugipon/src/tableSin]`: D_00638BBC..D_00638BC4, three words in ROM order 1.5707964f, 16385.0f (makeSinTable's step and divisor) and 10430.378f (makeArcSinTable's radian scale), all literals in the bodies, VMA 0x638BBC..0x638BC8, 12 B. The TU's whole pool, bounded by quaternion's word and FileManager's carve, so the blob resume row `[0x538BB8, lit4, cod/538BB8]` is gone.
- Defect removed: four pool words were declared `extern float D_0063....` and read through $gp. They are literals the developer wrote, interned by ee-as into each TU's own pool; the named extern asserted a global that never existed. Both objects' `.lit4` are byte-identical to the ROM run and the rebuilt ELF is unchanged.

### 2026-09-14 mig-lit4 pass 2: six single-run pools
- `[0x5391D8, .lit4, ico2/omori/src/ebrain]`: D_006391D8..D_006391DC, two words both 250000.0f, eBrainGetTarget's two chase-distance thresholds as literals, VMA 0x6391D8..0x6391E0, 8 B. The TU's whole pool; the blob resume row is gone and gv's words now start the next blob row.
- `[0x539394, .lit4, ico2/sugipon/src/act_a_p_1]`: D_00639394..D_0063939C, three words 10000.0f, 10000.0f, 40000.0f (walkAI's two jump-range tests, subAP1BrainMain's smoothing threshold) as literals, VMA 0x639394..0x6393A0, 12 B. The TU's whole pool, between a_p_1's and box's carves.
- `[0x53952C, .lit4, ico2/sugipon/src/handManager]`: D_0063952C, one word 0.1f, HandManager's hand-weight lerp rate as a literal, VMA 0x63952C..0x639530, 4 B. The TU's whole pool, between girl's and item's carves.
- `[0x539658, .lit4, ico2/sugipon/src/motionViewer]`: D_00639658..D_0063965C, two words both 0.01f, both owned by the still-asm modeMessage (its two `%gp_rel` relocs), as two ASM_LIT4_SLOT lines beside the stub, VMA 0x639658..0x639660, 8 B. This corrects the motionOrientManager row's note calling the pair unowned: they are motionViewer's, and the blob resume row is gone.
- `[0x5396FC, .lit4, ico2/sugipon/src/windField]`: D_006396FC..D_00639704, three words 0.1f, 0.1f, 0.01f; the first is a slot line for the still-asm ExecWindField, the other two are getParallelWindVector's and getRadiateWindVector's literals, VMA 0x6396FC..0x639708, 12 B. getRadiateWindVector reads the one word twice and gcc emits a single `li.s` for it, so the run stays three words. This also corrects the weapon row's note that 0x6396FC has no owner.
- `[0x539708, .lit4, ico2/sugipon/src/windManager]`: D_00639708, one word 0.1f, ExecWindManager's gust lerp rate as a literal, VMA 0x639708..0x63970C, 4 B. The TU's whole pool; the blob resume row at 0x5396FC is gone.
- Eleven pool externs removed. Every object's `.lit4` is byte-identical to its carved ROM run and the rebuilt ELF is unchanged.
