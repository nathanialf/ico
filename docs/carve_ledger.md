...
### 2026-09-14 chain D pass 30: act-env .lit4
- `[0x5397E8, .lit4, ico2/fumi/src/act-env]`: the TU's whole pool, 20 words, VMA 0x6397E8..0x639838, 80 B. The first two (-3000.0f and pi) belong to the matched environment test and are now literals in its body instead of `extern float` pool names; the eighteen after them are ACTGetEnvironment's, one per use site in ROM order, as slot lines beside its stub (each referenced exactly once and only from that function, per the pass 30 measurement). The blob resumes at `[0x539838, lit4, cod/539838]` for the words before way_kidnap's carve. Made at harvest so the function can own its literals when it lands.
