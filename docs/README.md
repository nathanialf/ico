...
An index, not a summary. Each file is listed once, with its purpose and its
status: **tracked** (in git), **local-only** (gitignored; on this machine the
`main` / `ntsc` / `aug6` worktrees share the same physical file, so an edit
here changes it on every branch) or **generated** (rewritten by a tool, never
hand-edit).

This is the project's one documentation directory. The former `decomp/` was
folded into it; a `decomp/...` path in an older commit message or note means
the same file here.

`main` targets PAL retail (SCES-50760). `CLAUDE.md` has the three-branch
layout and the matching rules; `tools/README.md` covers the tools.

## Where to look for X

| I want to... | Read |
|---|---|
| match a function, find the C for an asm shape | `docs/COOKBOOK.md` (shape-to-recipe, §1-13) |
...
| `COOKBOOK.md` | local-only | the shape-to-C-recipe lookup, sectioned §1-13. Canonical home for recipes; auto-memories cite its section numbers, so numbering is stable |
...
| `PROGRAMMERS.md` | tracked | per-programmer source dirs mapped to subsystems, and per-author idioms. Use it for idioms; the PAL listing supplies boundaries |
...
| `MATCHING_NOTES.md` | local-only | per-function deferred targets: what was tried, the residual, the untried hypothesis. Its header flags the USA/aug6-era provenance |
...
GitHub Pages publishes only `index.html`, `progress.json` and `PROGRESS.md`;
`.github/scripts/build_pages_site.sh` names that set in `SITE_FILES`.
Everything else here is working documentation and stays unpublished.

## Rules of the road for these docs

- **Mechanisms go in `NOTES.md`, recipes in `COOKBOOK.md`, per-function
  residue in `MATCHING_NOTES.md`.** A note in the wrong file will not be
  found.
- **Never document a crutch.** CLAUDE.md's ban covers anything that emits no
  bytes and exists to steer codegen. When a technique is banned, its
  mechanism may stay but its recipe goes.
- **A `func_00XXXXXX` is a per-target VMA.** It means nothing on another
  branch. Names from `config/symbol_addrs.pal.txt` travel; addresses do not.
