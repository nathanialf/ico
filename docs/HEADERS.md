...
They now live in `ico2/common/include/typedef.h`, the only attested header
every ico2 TU's `-I` order reaches; before the 2026-09-15 migration below they
were in the unattested `include/ico/types.h`.

A per-programmer header may claim one of those declarations, or one out of a
TU's file-top block, **only when the census shows that programmer's header is
where the code using it lived**: i.e. there are census rows pointing at that
header from the TUs in question.

Under that rule **nothing moved out of the cross-programmer set in this pass,
and no file-top extern block was emptied.**  The reason is measurable: of the 89
